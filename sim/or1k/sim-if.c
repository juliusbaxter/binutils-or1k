/* Main simulator entry points specific to OR1K
...
static void print_or1k_misc_cpu (SIM_CPU * cpu, int verbose);
static DECLARE_OPTION_HANDLER (or1k_option_handler);
...
/* GDB passes -E, even though it's fixed, so we have to handle it here. common code only handles it if SIM_HAVE_BIENDIAN is defined, which it isn't for or1k.  */
...
/* Handle or1k specific options.  */

static SIM_RC
or1k_option_handler (sd, cpu, opt, arg, is_command)
     SIM_DESC sd;
     sim_cpu *cpu;
     int opt;
     char *arg;
     ...
     char **argv;
{
  ...
  /* Open a copy of the cpu descriptor table.  */
  {
	...
	/* Set SM and FO bits in SR*/
	/* TODO - check if we're going to be executing from 0xf0000000, if so set EPH
	   at reset */
	or1k32bef_h_spr_set (cpu, SPR_SR, (SPR_SR_SM | SPR_SR_FO));

	/* clear GPRs */
	int j; for (j=0;j<32;j++) or1k32bef_h_gr_set (cpu, j, 0);
  ...
  /* Store in a global so things like or1k_dump_regs can be invoked
     ...
     char **envp;
{
  ...
  SIM_ADDR addr;
  /* TODO: Add command line option to override detected start address */
  if (abfd != NULL)
